...
ifeq (cleveldb,$(findstring cleveldb,$(superstruct_BUILD_OPTIONS)))
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=superstruct \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=superstructd \
...
ifeq (,$(findstring nostrip,$(superstruct_BUILD_OPTIONS)))
  ldflags += -w -s
endif
...
install: check-go-version go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/superstructd

build: check-go-version
	go build $(BUILD_FLAGS) -o bin/superstructd ./cmd/superstructd
...
build-reproducible-generic: go.sum
	...
	DOCKER_BUILDKIT=1 $(DOCKER) build -t latest-build-$(PLATFORM) \
		...
		-f Dockerfile .
	$(DOCKER) create -ti --name $(subst /,-,latest-build-$(PLATFORM)) latest-build-$(PLATFORM) superstructd
	$(DOCKER) cp -a $(subst /,-,latest-build-$(PLATFORM)):/usr/local/bin/superstructd superstructd_$(subst /,_,$(PLATFORM))
	sha256sum superstructd_$(subst /,_,$(PLATFORM)) >> ./superstructd_sha256.txt

# Add check to make sure we are using the proper Go version before proceeding with anything
check-go-version:
	@if ! go version | grep -q "go$(GO_VERSION)"; then \
		echo "\033[0;31mERROR:\033[0m Go version $(GO_VERSION) is required for compiling superstructd. It looks like you are using" "$(shell go version) \nThere are potential consensus-breaking changes that can occur when running binaries compiled with different versions of Go. Please download Go version $(GO_VERSION) and retry. Thank you!"; \
		exit 1; \
	fi
